...
if (!defined('PHP_MAJOR_VERSION')) {
	...
	add_action('admin_notices', function () {
        ...
        <div class="notice notice-error">
            <p><?php
				_e(
					'Media Cloud can\'t determine what version of PHP you are using as PHP_MAJOR_VERSION is not defined.  This is a serious server misconfiguration issue and you should contact your hosting provider.',
					'ilab-media-tools'
				);
				?></p>
        </div>
...
if (is_plugin_active('amazon-s3-and-cloudfront/wordpress-s3.php')) {
	...
	add_action('admin_notices', function () {
        ...
        <div class="notice notice-error">
            <p><?php
				_e(
					'Media Cloud cannot be activated the same time as <strong>Offload S3</strong>.  Please deactive one before activating the other.',
					'ilab-media-tools'
				);
				?></p>
        </div>
...
if (is_plugin_active('wp-stateless/wp-stateless-media.php')) {
	...
	add_action('admin_notices', function () {
        ...
        <div class="notice notice-error">
            <p><?php
				_e(
					'Media Cloud cannot be activated the same time as the <strong>WP-Stateless</strong>.  Please deactive one before activating the other.',
					'ilab-media-tools'
				);
				?></p>
        </div>
...
// Version Defines
define('MEDIA_CLOUD_VERSION', '4.6.4.1');
...
if (function_exists('media_cloud_licensing')) {
	...
	function media_cloud_licensing()
	{
		...
		if (!isset($media_cloud_licensing)) {
			...
			$media_cloud_licensing = fs_dynamic_init([
				'id'              => '1431',
	...
	media_cloud_licensing()->add_filter('permission_list', function ($permissions) {
		$permissions['feature-tracking'] = [
			...
			'desc'       => media_cloud_licensing()->get_text_inline(
				'Anonymously track which Media Cloud features are being used to allow us to prioritize development.',
				'permissions-plugin-features'
			),
			'priority'   => 50,
			'optional'   => true,
		];
	...
	if (!wp_doing_ajax() && !media_cloud_licensing()->is_activation_mode()) {
		...
		if (get_option('mcloud_show_wizard')) {
			...
			if (media_cloud_licensing()->is_network_active()) {
				Environment::UpdateNetworkMode(true);
